...
use crate::{GenGroup, NumField};

use super::{GenVectorSpace, VectorSpace};
...
impl<const N: usize, T: GenVectorSpace> std::ops::Deref for FiniteVS<N, T> {
  type Target = [T; N];
...
impl<const N: usize, T: GenVectorSpace> std::ops::DerefMut for FiniteVS<N, T> {
